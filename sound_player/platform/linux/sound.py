...
class LinuxPCMSound(BaseSound):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._needs_conversion = (
            self._file_sample_rate != self.config.sample_rate or self._file_channels != self.config.channels
        )

        # Cache whether float-to-int conversion is needed for _safe_read
        self._needs_float_conversion = self._file_is_float and self.config.dtype in (
            np.dtype(np.int16),
            np.dtype(np.int32),
        )
    ...
    def _do_play(self):
        ...
        if self._sound_file is None:
            ...
            if self._file_sample_rate != self.config.sample_rate or self._file_channels != self.config.channels:
                logger.debug(
                    f"File format mismatch: file={self._file_sample_rate}Hz/{self._file_channels}ch, "
                    f"config={self.config.sample_rate}Hz/{self.config.channels}ch"
                )
            ...
            if self._file_is_float:
                logger.debug(f"File is float format, will convert to {self.config.dtype}")
    ...
    def _get_converted_chunk(self, size: int) -> np.ndarray | None:
        ...
        result = np.zeros((size, self.config.channels), dtype=self.config.dtype)
        result_pos = 0
        ...
        while result_pos < size:
            ...
            if self._resample_buffer is None or self._resample_position >= len(self._resample_buffer):
                ...
                if data.ndim == 1:
                    data = data.reshape(-1, 1)
                if self._file_channels != self.config.channels:
